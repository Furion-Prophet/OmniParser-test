...
paddle_ocr = PaddleOCR(
  use_angle_cls=False,
  lang="ch",
  det_limit_side_len=960, # Intel Mac 建议上限 960，再高会卡死
)
...
def check_ocr_box(image_source: Union[str, Image.Image], display_img = True, output_bb_format='xywh', goal_filtering=None, easyocr_args=None, use_paddleocr=False):
    ...
    if use_paddleocr:
        ...
        if isinstance(raw_result, list) and len(raw_result) > 0 and 'res' in raw_result:
            actual_data = raw_result.get('res', [])
            # 强制转回旧版嵌套列表格式：[ [[坐标], (文字, 置信度)], ... ]
            formatted_list = []
            for item in actual_data:
                # 兼容性提取：优先取 points，没有则取 bbox 并转为 4 点格式
                pts = item.get('text_region', item.get('points'))
                txt = item.get('text', '')
                conf = item.get('confidence', 0)
                if pts:
                    formatted_list.append([pts, (txt, conf)])
            result = formatted_list # 包装一层以适配 OmniParser 循环
        else:
            result = raw_result[0] # 保持原样
        
        coord = []
        text = []
        for item in result:
          if len(item) < 2: continue
          if len(item[1]) < 2: continue
